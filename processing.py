...
def create_infotext(p, all_prompts, all_seeds, all_subseeds, comments=None, iteration=0, position_in_batch=0, use_main_prompt=False, index=None, all_negative_prompts=None):
    ...
    generation_params = {
        ...
        "Progressive Growing": "True" if getattr(p, 'enable_progressive_growing', False) else None,
        "Min Scale": p.progressive_growing_min_scale if getattr(p, 'enable_progressive_growing', False) else None,
        "Max Scale": p.progressive_growing_max_scale if getattr(p, 'enable_progressive_growing', False) else None,
        "Progressive Growing Steps": p.progressive_growing_steps if getattr(p, 'enable_progressive_growing', False) else None,
        "Refinement": "True" if p.progressive_growing_refinement and getattr(p, 'enable_progressive_growing', False) else None,
...
# Ensure default initialization of progressive growing attributes
    if not hasattr(p, 'enable_progressive_growing'):
        p.enable_progressive_growing = False
        p.progressive_growing_min_scale = 0.5
        p.progressive_growing_max_scale = 1.0
        p.progressive_growing_steps = 100
        p.progressive_growing_refinement = False
...
class StableDiffusionProcessingTxt2Img(StableDiffusionProcessing):
    ...
    def sample_progressive(self, conditioning, unconditional_conditioning, seeds, subseeds, subseed_strength, prompts):
        ...
        # 2) Линейная шкала масштабов без клампов SDXL:
        resolution_steps = np.linspace(min_scale, max_scale, int(self.progressive_growing_steps))

        # Вспомогательная функция: округление до кратности opt_f и минимальное значение opt_f
        def _snap(v: int) -> int:
            v_int = int(v)
            v_int = max(opt_f, v_int)                  # не даём упасть до 0
            v_int = (v_int // opt_f) * opt_f           # кратность латентной сетке
            return max(opt_f, v_int)

        # 3) Начальная стадия (больше НЕТ клампа 512/1536 для SDXL):
        initial_width = _snap(self.width * resolution_steps[0])
        initial_height = _snap(self.height * resolution_steps[0])

        # создаём шум под латенты размером H/8 x W/8
        ...
        # первый проход в txt2img-режиме на уменьшенном разрешении
        ...
        for i in range(1, total_stages):
            ...
            # ресэмплинг латентов на следующий размер
            ...
            # 5) (опц.) Рефайн на каждой стадии через короткий img2img
            if self.progressive_growing_refinement:
                # минимум 1 шаг, чтобы не получить 0 при малом steps
                steps_for_refinement = max(1, self.steps // total_stages)

                # Шум под текущий латент-размер
                ...
                # Декод и подготовка кондишенов для img2img
                ...
                # decode_latent_batch вернёт список тензоров (CHW) — собираем в батч
                decoded = torch.stack(decoded).float()
                decoded = torch.clamp((decoded + 1.0) / 2.0, 0.0, 1.0)      # [0..1]
                source_img = decoded * 2.0 - 1.0                             # [-1..1] как ожидает conditioning
